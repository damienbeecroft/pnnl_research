...
\bibliography{refs} % Entries are in the mybib.bib file
\end{document}


%The most basic algorithm for this code is given below.
%
%\begin{algorithm}
%\caption{Multifidelity Finite Basis Physics Informed Neural Network}\label{alg:cap}
%\begin{algorithmic}
%\Require $\Omega$, \verb|batch_generator|
%\State $nn_{sf} \gets$ \verb|train_single_fidelity_network()|
%\State $X \gets x$
%\State $N \gets n$
%\While{$N \neq 0$}
%\If{$N$ is even}
