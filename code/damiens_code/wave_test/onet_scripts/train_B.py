...
def gen_prev_data_B(u, model_A, params_A):
    predA = model_A.predict_u(params_A, u)
    predA_t = model_A.predict_ut(params_A, u)
    return predA, predA_t
...
if __name__ == "__main__":
    ...
    res_weight = 1
    ut_weight = 1
    save_str = "full_training_B"



    ymin = 0.3
    ymax = 0.4
    ics_coords = np.array([[ymin, 0.0],
                        [ymin, 1.0]])
    bc1_coords = np.array([[ymin, 0.0],
                            [ymax, 0.0]])
    bc2_coords = np.array([[ymin, 1.0],
                            [ymax, 1.0]])
    dom_coords = np.array([[ymin, 0.0],
                            [ymax, 1.0]])

    results_dir = "../results_full/"+save_str+"/"

    model_A = DNN_class(layers_branch_low, ics_weight, res_weight, ut_weight, lr)
    params_A = model_A.unravel_params(np.load(results_dir + '/params_A.npy'))

    # Create initial conditions samplers
    ics_sampler = DataGenerator_ICS(2, ics_coords, lambda x: gen_prev_data_B(x, model_A, params_A), batch_size)
