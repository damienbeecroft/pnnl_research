...
if __name__ == "__main__":
    ...
    d_vx = scipy.io.loadmat(net_data_dirB + "beta_test.mat")
    uB, predB= (d_vx["U_res"].astype(np.float32), d_vx["S_pred"].astype(np.float32))
    ...
    fig3, gx = plt.subplots(figsize=(6,3))
    plt.figure(fig3.number)
    plt.plot(y, s[:, 0], 'k', linestyle=':', linewidth=2, label='Exact')
    i=0
    plt.plot(uA[:, i], predA[:, i],  '#59a14f', linestyle='-', label='SF prediction')
    plt.plot(uB[:, i], predB[:, i],  '#e15759', linestyle='--', label='MF correction', linewidth=2)
 ...
 #   plt.plot(uF[:, i], predF[:, i], '#4e79a7', linestyle='-', label='F prediction')
    ...
    i=1
    plt.plot(uA[:, 0], predA[:, i],  '#59a14f', linestyle='-', label='SF prediction')
    plt.plot(uB[:, 0], predB[:, i],  '#e15759', linestyle='--', label='MF correction', linewidth=2)
