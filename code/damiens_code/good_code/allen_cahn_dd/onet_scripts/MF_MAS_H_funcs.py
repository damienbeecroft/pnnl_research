...
class MF_DNN_class:
    ...
    # Initialize the class
    def __init__(self, layers_branch_nl, layers_branch_l,  layers_lf, ics_weight, res_weight, ut_weight, lr,
                Ndomains, delta, dom_coords, params_A, restart =0, params_t = 0): 
    ...
    def w_jl(self, j, l, u):
        ...
        mu = self.dom_lens[0]*j/(l-1)
        sigma = self.dom_lens[0]*(self.delta/2.0)/(l-1)
    ...
    def operator_net(self, params, x, t):
        ...
        for level in self.params_t:
            ...
            for mfparams in level:
                ...
                w = self.w_jl(i, self.Ndomains[j], t)
                weight_sum += w
                ul_cur += w*(u_l + u_nl)
                i +=1
